...
int main(int argc, char* argv[]) {
    ...
    for(size_t t = 0; t < num_timesteps; ++t) {
        for(size_t r = 0; r < num_robots; ++r) {
            ...
            if(t < num_timesteps - 1) {
                run_data.vx[t][r] = (positions[r][2*(t+1)] - positions[r][2*t]) / R_DT;
                run_data.vy[t][r] = (positions[r][2*(t+1)+1] - positions[r][2*t+1]) / R_DT;
            }
        }
    }
    ...
    auto fields = ChampVectorielGenerateur:: ChampVectorielGenerateur::generate_single_run_fields(
                run_data, OPTIMAL_W);
    ...
    wasserstein_params.wasserstein_power = 2.0;
